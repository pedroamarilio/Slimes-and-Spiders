...
class Animation(gameimage.GameImage):
    ...
    def __init__(self, image_file, total_frames, loop=True):
        ...
        # allow horizontal flipping of frames
        self.flip_x = False
    ...
    def draw(self):
        if(self.drawable):
            ...
            clip_rect = pygame.Rect(int(self.curr_frame * self.width),
                                    0,
                                    int(self.width),
                                    int(self.height)
                                    )
            ...
            try:
                if getattr(self, 'flip_x', False):
                    # extract subsurface and flip horizontally
                    subsurf = self.image.subsurface(clip_rect).copy()
                    subsurf = pygame.transform.flip(subsurf, True, False)
                    window.Window.get_screen().blit(subsurf, self.rect)
                else:
                    window.Window.get_screen().blit(self.image, self.rect, area=clip_rect)
            except Exception:
                # fallback to original blit in case of error
                window.Window.get_screen().blit(self.image, self.rect, area=clip_rect)
