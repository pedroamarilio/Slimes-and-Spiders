...
def run_game(janela, musica1):
    ...
    while True:
        ...
        # periodic income from goldmines
        ...
        # update wave manager only when not in game-over pause
        ...
        # draw frame; when game over is active, draw without presenting, then draw overlay and present once
        screen.update(economy, entities, selection_area, dt, menu_atual, controller.toPlace, wave_manager, present=(not game_over_active))

        # Debug key to force Game Over
        try:
            if not game_over_active and keyboard.key_pressed('Z'):
                game_over_active = True
                game_over_start = pygame.time.get_ticks()
                try:
                    gameover_sprite = Sprite('assets/sprites/gameover.png', 1)
                    gw = getattr(janela, 'width', 1366)
                    gh = getattr(janela, 'height', 768)
                    gx = int((gw - gameover_sprite.width) / 2)
                    gy = int((gh - gameover_sprite.height) / 2)
                    gameover_sprite.set_position(gx, gy)
                except Exception:
                    gameover_sprite = None
                try:
                    screen.set_hud_message("GAME OVER", 10.0)
                except Exception:
                    pass
        except Exception:
            pass

        # Normal detection of game over
        if not game_over_active:
            try:
        ...
        # If in game-over state, draw overlay and present once; after 10s return to menu
        if game_over_active:
            try:
                if gameover_sprite is not None:
                    gameover_sprite.draw()
                else:
                    # HUD message already set; nothing else to draw
                    pass
            except Exception:
                pass
            janela.update()
            ...
            try:
                if pygame.time.get_ticks() - game_over_start >= 10000:
                    return
            except Exception:
                pass
...
# main loop: show menu then run game session
while True:
    try:
        ...
        # if menu fails, still try to run
        pass

    # start a game session; returns to menu after game over
    run_game(janela, musica1)
